...
void BraveSyncServiceImpl::BackgroundSyncStarted(bool startup) {
  ...
  const bool waiting_for_second_device =
                            sync_prefs_->GetSyncDevices()->have_less_than_two();
...
void BraveSyncServiceImpl::OnResolvedPreferences(const RecordsList& records) {
  ...
  auto sync_devices = sync_prefs_->GetSyncDevices();
  const bool waiting_for_second_device = sync_devices->have_less_than_two();
  ...
  if (this_device_deleted ||
                 (at_least_one_deleted && sync_devices->have_less_than_two())) {
    ResetSyncInternal();
  } else if (waiting_for_second_device && !sync_devices->have_less_than_two()) {
